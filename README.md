...
2. In the Console, create a VectorsDB database with ID `recall`, then a collection with ID `memories` using the `bge-small` (384) embedding model, and add an `hnsw_cosine` index on the `embeddings` field.
...
6. Install and deploy the MCP server (uses the [Appwrite CLI](https://appwrite.io/docs/tooling/command-line/installation)):
